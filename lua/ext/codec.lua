...
function exports.wrapStream(socket)
    ...
    function onRead(err, chunk)
        ...
        if waiting then
            local thread = waiting
            waiting = nil
            assert(coroutine.resume(thread, table.unpack(data)))
        else
            queue[#queue + 1] = data
            if not paused then
                paused = true
...
function exports.chain(...)
    ...
    return function(read, write)
        local threads = { }
        -- coroutine thread for each item
        local waiting = { }
        -- flag when waiting to pull from upstream
        local boxes = { }
        -- storage when waiting to write to downstream
        for i = 1, nargs do
